...
library(knitr)
library(kableExtra)
>>>>>>> bacd7c695aea6ec066d91fde1ae404e8c4279b44
...
mean_psa_df <- data.frame(
  Treatment_Type = c("1 (Surgery)", "2 (RT)", "3 (Hormone Therapy)", "4 (RT + Hormone)"),
  Mean_Delta_PSA = c(53.905, 68.503, 83.659, 70.428)
)

mean_psa_df %>%
  kable(caption = "Mean PSA Reduction (Delta PSA) by Treatment Type",
        booktabs = TRUE,
        linesep = "",
        col.names = c("Treatment Type",
...
tukey_data <- data.frame(
  Group_1 = c("1 (Surgery)", "1 (Surgery)", "1 (Surgery)", "2 (RT)", "2 (RT)", "3 (Hormone Therapy)"),
  Group_2 = c("2 (RT)", "3 (Hormone Therapy)", "4 (RT + Hormone)", "3 (Hormone Therapy)", "4 (RT + Hormone)", "4 (RT + Hormone)"),
...
tukey_data %>%
  kable(caption = "Table 4.3: Tukey's HSD Post-Hoc Test for Delta PSA",
        booktabs = TRUE,
        linesep = "",
        col.names = c("Group 1", "Group 2", "Mean Difference", "p-value", "Significant (p < 0.05)"),
        align = c('l', 'l', 'r', 'r', 'c')) %>%
  kable_styling(latex_options = c("hold_position", "scale_down"))
...
Since the p-value (2.66e-06) is less than the significance level (a=0.05), we reject the null hypothesis (H0). There is a statistically significant difference in the mean change in PSA (Delta PSA) among the different treatment groups.
