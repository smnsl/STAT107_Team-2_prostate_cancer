...
cat(sprintf("AUC (Area Under the Curve, Train Set): %.4f\n", auc_score_train))
...
# Train ROC
plot(roc_obj_train, 
     main = "ROC Curve for Training Set (Oversampling)",
...
# Test ROC
plot(roc_obj, 
     main = "ROC Curve for BCR Prediction Model (Oversampling)",
     xlab = "False Positive Rate (1 - Specificity)", 
     ylab = "True Positive Rate (Sensitivity)",
     lwd = 3)
...
Upon testing the model on the unseen data, the Area Under the Curve (AUC) was calculated to be 0.5685, indicating that the model's predictive capability is only slightly better than random chance.
...
The confusion matrix yields an overall accuracy of 56.11%. With a sensitivity of 52.73% and a specificity of 57.60%, the model struggles to reliably classify positive BCR cases, suggesting that while PSA and Treatment are significant risk factors, they are not sufficient on their own to form a high-precision predictive tool.
