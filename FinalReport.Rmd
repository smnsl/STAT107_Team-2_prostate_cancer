...
final_table <- model_tidy %>%
  select(term, estimate, conf.low, conf.high, p.value) %>%
  
mutate(
    `95% CI` = paste0("(", round(conf.low, 3), ", ", round(conf.high, 3), ")"),
    `P-Value` = format.pval(p.value, digits = 4, eps = 1e-05, scientific = FALSE, stars = TRUE),
...
final_table %>%
  kable(
    caption = "Multivariate Logistic Regression Results for Biochemical Recurrence (BCR)",
    booktabs = TRUE,
    linesep = "",
    align = c('l', 'r', 'c', 'c') 
  ) %>%
  ...
  footnote(
    general = "Odds Ratios (OR) and 95% Confidence Intervals are presented. OR > 1 indicates increased odds of BCR. *** p < 0.001, ** p < 0.01, * p < 0.05.",
    threeparttable = TRUE
  )
